...
class Relation(pd.DataFrame):
    ...
    def select(self, q):
        ...
        >>> from reframe import Relation
        >>> country = Relation('country.csv')
        >>> country.select('continent == "Antarctica"').project(['code','name'])
            code                                          name
        232  ATA                                    Antarctica
    ...
    def sort(self, *args, **kwargs):
        ...
        >>> from reframe import Relation
        >>> country = Relation('country.csv')
        >>> country.sort(['indepyear'], ascending=False).select('indepyear < 1200').project(['name','indepyear'])
                       name  indepyear
        159        Portugal       1143
    ...
    def intersect(self, other):
        ...
        >>> from reframe import Relation
        >>> country = Relation('country.csv')
        >>> country.select('continent == "Africa"').project(['name', 'region']).intersect(country.select('region == "Western Africa"').project(['name', 'region']))
                     name          region
    ...
    def njoin(self, other):
        ...
        >>> from reframe import Relation
        >>> country = Relation('country.csv')
        >>> country.select('continent == "Africa"').project(['name', 'region']).njoin(country.select('region == "Western Africa"').project(['name', 'region']))
                     name          region
        0           Benin  Western Africa
        1    Burkina Faso  Western Africa
        2          Gambia  Western Africa
    ...
    def leftouterjoin(self, other):
        ...
        if not col_list:
            raise ValueError("The two relations must have some columns in common")
        return Relation(pd.merge(self,other, on=list(col_list),how='left'))

    def rightouterjoin(self, other):
        col_list = [x for x in self.columns if x in other.columns]
        if not col_list:
            raise ValueError("The two relations must have some columns in common")
        return Relation(pd.merge(self,other, on=list(col_list),how='right'))
    def union(self,other):
        ...
        >>> from reframe import Relation
        >>> country = Relation('country.csv')
        >>> country.select('region == "Western Africa"').union(country.select('region == "Polynesia"')).project(['name','region'])
                          name          region
    ...
    def minus(self,other):
        ...
        >>> from reframe import Relation
        >>> country = Relation('country.csv')
        >>> country.select('continent == "Africa"').minus(country.select('region == "Western Africa"')).project(['name','region','continent'])
    ...
    def count(self):
        return Relation(pd.DataFrame([[self.__len__()]], columns={'count'}))

    def min(self, col):
        return Relation(pd.DataFrame([[self[col].min()]], columns={'min'}))

    def max(self, col):
        return Relation(pd.DataFrame([[self[col].max()]], columns={'max'}))
    def avg(self, col):
        return Relation(pd.DataFrame([[self[col].avg()]], columns={'avg'}))
